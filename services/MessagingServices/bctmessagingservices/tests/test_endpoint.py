...
from bctmessagingservices.tests.utils import create_messaging_channel, create_endpoint_sut
...
class TestMessagingEndPointControllerExceptions(unittest.TestCase):
    ...
    def test_no_service_is_defined(self):
        env = environment.create()
        method_name = 'service_method'

        mq, request, self.properties = create_endpoint_sut(None, method_name)

        self.reply = mq.published_messages[0]

        jsonrpc_response = json.loads(self.reply.body)
        del jsonrpc_response['error']['message']
        response_should_be = {
                'jsonrpc': '2.0',
                'id': self.properties.correlation_id,
                'error': {
    ...
    def test_method_does_not_exist(self):
        ...
        mq, request, self.properties = create_endpoint_sut(ServiceDouble(), method_name)
